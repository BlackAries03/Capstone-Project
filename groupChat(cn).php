...
if (!isset($groupID) || !is_numeric($groupID)) {
    ...
    setTimeout(function() {
        window.location.href = "message(cn).php"; 
    }, 250); 
    </script>';
    exit();
}
...
<body>
    ...
    <div class="main-content">
        <header class="header">
            <button class="return-button" onclick="window.location.href='message(cn).php'">
        ...
        <div class="container">
            ...
            <footer class="chat-footer">
                <form id="chatGroupForm">
                    ...
                    <textarea id="messageGroupInput" name="message" placeholder="输入信息。。" rows="4"></textarea>
            ...
            <div class="popup-content">
                ...
                <?php if ($userRole == 'creator'): ?>
                    <div class="input-group">
                        <label for="groupImage"> 群组图片</label>
                    ...
                    <div class="input-group">
                        <label for="addMember">添加成员</label>
                        <input type="text" id="addMember" placeholder="输入成员名称" />
                    </div>
                    <div class="friends-list" style="height: 100px; width:100%; overflow-y: scroll; overflow-x: hidden;">
                        ...
                        if (!empty($mutualFollows)):
                            ...
                            ?>
                            <p id="no-result" style="color:red;">未找到其他共同用户。</p>
                        <?php endif; ?>
                    </div>

                    <div class="input-group">
                        <label for="MemberList">成员列表</label>
                        <input type="text" id="MemberList" placeholder=" 输入成员名称" />
                    </div>
                    <div class="friends-list" style="height: 100px; width:100%; overflow-y: scroll; overflow-x: hidden;">
                        <?php
                        $groupMembers = [];
                        ...
                        if (!empty($groupMembers)):
                            ...
                            ?>
                            <p id="no-result" style="color:red;">群组中未找到成员。</p>
                        <?php endif; ?>
                    </div>
                <?php else: ?>
                    <div class="input-group">
...
<script>
    ...
    function removeMember(userID, username, element) {
        if (confirm(`确认把${username} 从群组中移除吗?`)) {
            const groupID = new URLSearchParams(window.location.search).get("groupID");
    ...
    function transferRole(userID, username, element) {
        if (confirm(`确认把创建者角色转移给${username}吗?`)) {
            const groupID = new URLSearchParams(window.location.search).get("groupID");
